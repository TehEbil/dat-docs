---
title: "Ändern einer Bewertung"
topic_id: "15170"
breadcrumb: "SilverDAT Produkte > SilverDAT 3 PRO > Schnittstellenoperationen > Bewertung > Verwalten von Vorgängen > Ändern einer Bewertung"
---

# Ändern einer Bewertung

Mit der Funktion changeValuationN ändern Sie einen gespeicherten Vorgang. 
   Um einen Vorgang zu ändern, benötigen Sie mindestens die Identifikationsnummer des
      Auftrags contractID und das Dossier Element (Aktenzeichen Vorgang).  Die Identifikationsnummer entspricht der DossierId im Response der Funktion createValuationN. 
   Variante mit unvollständiger Fahrzeugidentifikation
   Die Funktion changeValuationN können Sie in der Variante mit unvollständiger Fahrzeugidentifikation einsetzen.
      In diesem Fall ändern Sie einen Vorgang ohne das Fahrzeug vollständig zu definieren.
      Die unvollständige Fahrzeugidentifikation erkennt man anhand der Response "incomplete Evaluation".
   Voraussetzungen:
   
      
         Der zu ändernde Vorgang muss sich im Mode Variante mit unvollständiger Fahrzeugidentifikation
            befinden.
         ...
         Um den Mode Variante mit unvollständiger Fahrzeugidentifikation beizubehalten, darf
            der Request keine oder nur unvollständige Fahrzeugdefinitionen beinhalten.
      
   
   Request
   Das nachfolgende Beispiel zeigt ein Dossier Element mit den relevanten Pflichtfeldern, die im Standardfall mindestens benötigt
      werden. 
   Beispiel:
   
            <vxs:Dossier>
            <vxs:Name>MyChangedDossier</vxs:Name>
...
         </vxs:Dossier>
   ...
   Details zur Funktionsweise im Standardfall
   Es werden nur die nachfolgend beschriebenen Parameter unterstützt, nicht erwähnte
      Parameter werden ignoriert. Parameter, die im Request nicht angegeben werden, führen
      normalerweise nicht zu einer Änderung. 
   Ausnahmen:
   
      
         Wird ein Equipment-Element angegeben, dann muss auch die gesamte Liste der Ausstattungen übergeben werden.
            Fehlende Unterelemente löschen gegebenenfalls gespeicherte Werte.
      
      
         Wird ein Condition-Element angegeben, dann müssen auch alle zugehörigen Unterelemente angegeben werden.
            Fehlende Unterelemente löschen gegebenenfalls gespeicherte Werte.
      
   
   Falls mindestens einer der Parameter: DAT €uropa-Code®, Marktindex oder Bauzeit geändert wird, dann muss eine erneute Identifikation des
      Fahrzeugs durchgeführt werden. Das bedeutet, dass diese drei Parameter immer zusammengehörig
      angegeben werden müssen. Eine erneute Identifikation des Fahrzeugs bewirkt die nachfolgend
      beschriebenen Auswirkungen:
   
      
         Die gespeicherte Liste der Ausstattungen wird gelöscht und neu initialisiert.
      
      
         Das Ergebnis einer zuvor durchgeführten VIN-Abfrage wird gelöscht. Die VIN selbst
            bleibt gespeichert, wird jedoch als deaktiviert gekennzeichnet.
      
   
   Details zur Funktionsweise für die Variante mit unvollständiger Fahrzeugidentifikation
   Bei der Variante mit unvollständiger Fahrzeugidentifikation müssen immer alle Parameter
      erneut übergeben werden, ein Hinzufügen zu bestehenden Werten ist in diesem Fall nicht
      möglich. Wird das Fahrzeug vollständig definiert, dann wird automatisch der Mode mit
      unvollständiger Fahrzeugidentifikation beendet. Es ist danach nicht mehr möglich wieder
      in den Mode mit unvollständiger Fahrzeugidentifikation zurück zu gelangen. 
   Rückgabe
   Im Erfolgsfall wird der Vorgang als VXS zurückgegeben, andernfalls wird eine Fehlermeldung zurückgegeben. Die Beschreibung
      ...
      DossierId ist identisch mit der contractID die Sie zum Beispiel in der Funktion getValuationN benötigen. Ob es sich um einen Response der Variante mit unvollständiger Fahrzeugidentifikation
      handelt können Sie anhand des Werts incompleteValuation im Attribut type des Elements VXS erkennen.
   Parameter
   Es werden nur die nachfolgend beschriebenen Parameter unterstützt, nicht erwähnte
      Parameter werden ignoriert. Parameter, die im Request nicht angegeben werden, führen
      normalerweise nicht zu einer Änderung. 
   Standardfall
   Request changeValuationN
                  ...
                  Eindeutige Identifikationsnummer des Auftrags. Die Identifikationsnummer contractID ist identisch mit der DossierId aus createValuationN.
   ...
   Element vxs:Dossier
                  ...
                  Enthält die Informationen zu den Handelstätigkeiten 
                  (Coverage=EXTENDEDBYTRADING)
   ...
   Request changeValuationN
                  ...
                  Eindeutige Identifikationsnummer des Auftrags. Die Identifikationsnummer contractID ist identisch mit der DossierId aus createValuation.
